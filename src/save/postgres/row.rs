...
/// Lookup (position-aware): (obs, abs, seat_position) -> isomorphism table
#[async_trait::async_trait]
impl Row for (i64, i16, i16) {
    async fn write(self, writer: Pin<&mut BinaryCopyInWriter>) {
        writer //   (obs,     abs,     seat_position)
