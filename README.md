...
`robopokermulti` is a follow-on fork of `robopoker` focused on multiway NLHE
blueprint training, large-table cash play, and tournament extensions.

This repo starts from the `robopoker` heads-up baseline, but it is not trying to
stay wire-compatible or architecture-compatible with that line forever. The
purpose of the fork is to let the multiway engine, abstraction system, infoset
schema, and training stack evolve cleanly.

# Overview

This repo starts from the existing `robopoker` research base and extends it
toward:

1. Multiway cash blueprint training up to 10 seats
2. Profile-aware clustering, training, and inference
3. Tournament utility support, followed by tournament-state extensions

The current first-priority milestone is a stable multiway cash-game foundation.
Tournament state, table balancing, and deeper serving/runtime concerns come
after that base is validated.
