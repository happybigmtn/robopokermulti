...
impl Metric {
    ...
    pub async fn from_street_profile(
        ...
        let metric_table = if tables.is_default_v1() {
            crate::save::METRIC.to_string()
        } else {
            tables.metric()
        };
        let sql = format!("SELECT tri, dx FROM {metric_table}");
        let mut keys = std::collections::HashSet::new();
        for ref x in Abstraction::all(street) {
            for ref y in Abstraction::all(street) {
                if x < y {
    ...
    pub async fn stream_profile(
        ...
        let copy = format!("COPY {metric_table} (tri, dx) FROM STDIN BINARY");
        ...
        let mut writer = Box::pin({
            ...
            BinaryCopyInWriter::new(sink, <Metric as Schema>::columns())
        });
        ...
        for row in self.rows() {
            ...
            if in_chunk >= chunk_size {
                ...
                log::info!(
                    "~ metric copy: committed {} rows (total {})",
                    in_chunk,
                    total
                );
                writer = Box::pin({
        ...
        writer.as_mut().finish().await.expect("finish");
        log::info!(
            "~ metric copy: committed final {} rows (total {})",
            in_chunk,
            total
        );
    }
    ...
    pub async fn truncate_profile(
        ...
        let sql = format!("TRUNCATE TABLE {metric_table};");
        client.batch_execute(&sql).await.expect("truncate metric");
    }
    ...
    pub async fn finalize_profile(
        ...
        let sql = format!(
            "INSERT INTO {metric_table} (tri, dx) VALUES (0, 0) ON CONFLICT DO NOTHING;
             CREATE INDEX IF NOT EXISTS idx_{metric_table}_tri ON {metric_table} (tri);
             CREATE INDEX IF NOT EXISTS idx_{metric_table}_dx  ON {metric_table} (dx);
             ALTER TABLE {metric_table} SET (fillfactor = 100);
             ALTER TABLE {metric_table} SET (autovacuum_enabled = false);"
        );
        client.batch_execute(&sql).await.expect("finalize metric");
    }
}
