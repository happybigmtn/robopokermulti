...
/// The walker rotates across all players: `Turn::Choice(epoch % player_count)`.
pub struct Worker {
    client: Arc<Client>,
    nodes: AtomicUsize,
    epoch: AtomicUsize,
    ...
    profile: Option<Arc<crate::save::TrainingProfileConfig>>,
}

impl Worker {
    /// Create a new worker for heads-up (2-player) training.
    pub fn new(client: Arc<Client>) -> Self {
        Self::for_players(client, 2, crate::save::TrainingTables::default_hu())
    }

    /// Create a new worker for N-player training.
    pub fn for_players(
        client: Arc<Client>,
        player_count: usize,
        tables: crate::save::TrainingTables,
    ) -> Self {
        Self::for_profile(client, player_count, tables, None)
    }

    /// Create a new worker with an optional training profile configuration.
    pub fn for_profile(
        client: Arc<Client>,
        player_count: usize,
        tables: crate::save::TrainingTables,
        ...
        let expected = profile.as_ref().map(|cfg| cfg.player_count);
        if let Some(expected) = expected {
            assert!(
                player_count == expected,
                "player_count must match training profile"
            );
        }
        assert!(
            player_count >= 2 && player_count <= 10,
            "player_count must be 2-10"
        );
        Self {
            client,
    ...
    fn walker(&self) -> Turn {
        Turn::Choice(self.epoch() % self.player_count)
    }
...
impl Worker {
    ...
    pub async fn step(&self) {
        self.client
            .submit_profile(&self.tables.profile, self.batch().await)
            .await;
        self.client.advance_profile(&self.tables.profile).await;
        self.inc_epoch();
    }
}
...
impl Worker {
    ...
    async fn memory(&self, info: &Info) -> Memory {
        self.client
            .memory_profile(&self.tables.profile, *info)
            .await
    }
...
impl Worker {
    pub async fn tree(&self) -> Tree<Turn, Edge, Game, Info> {
        ...
        let config = self
            .profile
            .as_ref()
            .map(|cfg| cfg.table_config_for_epoch(self.epoch()))
            .unwrap_or_else(|| TableConfig::for_players(self.player_count));
        let ref root = Game::root_with_config(config);
        let mut tree = Tree::default();
        let node = tree.seed(self.seed(root).await, Game::root_with_config(config));
